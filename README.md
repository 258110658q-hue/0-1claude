...
# 编辑 .env，填入你的 API Key 和 MODEL_ID

# 3. 自检 + 启动
python app.py --check           # 确认配置正确
python app.py                   # 启动
```
...
```bash
python app.py --check     # 自检：.env 配置是否正确
python app.py --test      # 运行 194 个单元测试
python app.py             # 正常启动交互模式
```
...
| 问题 | 原因 | 解决 |
|------|------|------|
| `KeyError: 'MODEL_ID'` | `.env` 不存在或配置错误 | `cp .env.example .env` 后编辑填入真实值 |
| `ModuleNotFoundError` | 依赖未安装 | `pip install -r requirements.txt` |
| API 返回 401 | API Key 错误 | 检查 `.env` 中的 `ANTHROPIC_API_KEY` |
| `date` 命令卡住 | LLM 不知道在 Windows 上 | 告诉它 `这是 Windows 系统` |
