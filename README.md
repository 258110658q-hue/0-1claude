...
- Python 3.10+
- Git（worktree 功能需要）
- 一个支持 Anthropic Messages API 的 API Key
...
| 方式 | 说明 | 获取地址 |
|------|------|---------|
| **Anthropic 官方** | 直接使用 Claude 系列模型 | [console.anthropic.com](https://console.anthropic.com/) → API Keys |
| **DeepSeek** | 国内可直接注册，兼容 Anthropic API | [platform.deepseek.com](https://platform.deepseek.com/) → API Keys |
| **中转平台** | 第三方代理，支持多种模型 | 各平台自行提供 |
...
在项目根目录创建 `.env` 文件（已加入 `.gitignore`，不会上传）：
...
如果用的是 DeepSeek 或中转平台，还需要加一行 `ANTHROPIC_BASE_URL`：
...
以下是 5 个复制粘贴就能跑的测试命令，从简单到复杂：

```bash
# 1. 基础对话：让 LLM 写代码
s20 >> 用 Python 写一个快速排序

# 2. 读写文件：Agent 会调用 read_file 工具
s20 >> 读一下 core/utils.py 的前 15 行

# 3. 任务规划：Agent 会调用 todo_write
s20 >> 帮我规划给项目加日志系统的步骤，列出来就行

# 4. 子 Agent：启动独立 Agent 做复杂扫描
s20 >> 用子 Agent 统计 core/ 目录下每个 .py 文件有多少个函数
...
| 问题 | 原因 | 解决 |
|------|------|------|
| `KeyError: 'MODEL_ID'` | `.env` 文件不存在或路径不对 | 确认 `.env` 在 `0-1claude/` 根目录 |
| `模块未找到: anthropic` | 依赖未安装 | `pip install anthropic python-dotenv pyyaml` |
| 启动后没反应 | Windows 终端编码问题 | 不用管，输入英文或数字试试 |
| API 返回 401 | API Key 错误 | 检查 `.env` 中的 key 是否正确 |
| `date` 命令卡住 | LLM 不知道在 Windows 上 | 告诉它 "这是 Windows 系统，不要用 date 命令" |
